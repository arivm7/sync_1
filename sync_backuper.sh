...
##
##  Project     : sync_1
##  Description : Скрипт для простого резервного копирования.
...
Опции:
  ...
  --usage, -u        Показать краткую справку по использованию
  --version, -V      Показать версию
  --edit-conf, -ec   Редактирование конфига
  --edit-list, -el   Редактирование списка для бакапа
...
check_dependencies() {
    ...
    for cmd in $DEPENDENCIES_REQUIRED; do
        if ! command -v "$cmd" &>/dev/null; then
            exit_with_msg "Ошибка: программа '$cmd' не найдена. Установите её, пожалуйста." 1
        fi
    done

    for cmd in $DEPENDENCIES_OPTIONAL; do
        if ! command -v "$cmd" &>/dev/null; then
            echo "ℹ️ Предупреждение: программа '$cmd' не найдена. Некоторые функции будут недоступны."
        fi
    done
}
