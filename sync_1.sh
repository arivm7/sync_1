...
LAST_CHANGES="\
v1.3.0 (2025-04-21): Дабавлена команда автоматического создания удалённого репозитория командой CLOUD_UP_INIT
v1.3.1 (2025-04-22): Дабавлено дефолтное наполнение файла excludes
...
# Список команд, тебующих наличия папки .sync
...
#
exit_with_msg() {
    local msg="${1:?exit_with_msg строка не передана или пуста. Это программная ошибка скрипта. Смотреть вызывающую функцию.}"
    local err="${2:-1}"
    logger -p error "${LOG_PREFIX} ERR: ${msg}"
    if [ "$err" -eq 2 ]; then
        msg="${msg}\nПодсказка по использованию: \e[1;32m${APP_NAME} --usage|-u\e[0m"
    fi
    echo -e "${msg}"
    exit "$err"
}
...
Использование:
    ...
    ${SYNC_CMD_PAUSE}   -- Обмен данными не происходит. Режим используется для изменений данных 
               на сервере. Никакая комманда с серера ничего не скачивает. 
    ...
    ${SHOW_DEST} -- Показать строку "dest" (из файла "${FILE_DEST}").
               Это адрес размещения папаки на облачном сервере. Обычно вида "user@host/путь/папка".
...
validate_cloud_up_init() {
    local remote_spec="${1:?check_cloud_dir: укажите путь вида user@host:/path/to/new_dir}"
    ...
    # Проверка родительской папки через check_cloud_dir (тихий режим для удобства)
    if ! check_cloud_dir "${remote_user_host}:${remote_parent_path}"; then
        echo "Ошибка: родительская папка '$remote_parent_path' недоступна или не существует на сервере '$remote_user_host'."
        return 1
    fi

    # Проверка отсутствия конечной папки
...
find_sync_dir() {
    local current
    current=$(get_abs_path "$DIR_LOCAL")
    while [[ "$current" != "/" ]]; do
...
parse_args() {
    ...
    # Проверка команды CLOUD_UP_INIT
        ...
        if ! validate_cloud_path "${DIR_CLOUD}"; then
...
dl()
{
    ...
    if [ "${TYPE_TRANSFER}" = "${SYNC_TYPE_SERVICE}" ]; then
        ...
        # local EXEC="${CMD_TRANSFER_SERV:?CMD_TRANSFER_SERV не инициализирована.}"
    else
        # echo "Копирование данных"
        "${CMD_TRANSFER_DATA[@]}" "${LOCAL}" "${DEST}"
        # local EXEC="${CMD_TRANSFER_DATA:?CMD_TRANSFER_DATA не инициализирована.}"
    fi
    # ${EXEC} --rsh="ssh -p ${SSH_PORT}" "${LOCAL}" "${DEST}"
...
set_status_my()
{
    ...
    printf "Установка статуса синхронизации в \"${STATUS}\"... "
    echo "${STATUS}" > "${DIR_LOCAL}/${DIR_SYNC}/${MY_NAME}"
...
set_status_all()
{
    ...
    echo "Установка для всех статуса синхронизации в \"${STATUS_ALL}\" (для меня в \"${STATUS_MY}\")"
    init_temp
...
do_cloud_up_init()
{
    ...
    {
        PREV_DIR=$(pwd)
        cd "${DIR_LOCAL}" || { echo "Не удалось перейти в папку [${DIR_LOCAL}]"; exit 1; }
        DIR_LOCAL=$(pwd)
        LOCAL_PATH=$(dirname "${DIR_LOCAL}")
        LOCAL_FOLDER=$(basename "${DIR_LOCAL}")
        cd "${PREV_DIR}" || { echo "Не удалось перейти в папку [${PREV_DIR}]"; exit 1; }
    }
    ...
    mkdir -p "${DIR_LOCAL}/${DIR_SYNC}" || { echo "Не удалось создать папку синхронизатора [${DIR_LOCAL}/${DIR_SYNC}]"; exit 1; }
    ...
    mkdir -p "${DIR_LOCAL}/${DIR_TEMP}/${CLOUD_FOLDER}/${DIR_SYNC}" || { echo "Не удалось создать папку [${DIR_LOCAL}/${DIR_TEMP}/${CLOUD_FOLDER}/${DIR_SYNC}]"; exit 1; }
    # shellcheck disable=SC2059
    printf   "Копируем [${CLOUD_FOLDER}/${DIR_SYNC}/${MY_NAME}]"
    cp       "${DIR_LOCAL}/${DIR_SYNC}/${MY_NAME}" "${DIR_LOCAL}/${DIR_TEMP}/${CLOUD_FOLDER}/${DIR_SYNC}/"
    # shellcheck disable=SC2059
    printf   ", [${CLOUD_FOLDER}/${FILE_DEST}]"
    cp       "${DIR_LOCAL}/${FILE_DEST}"         "${DIR_LOCAL}/${DIR_TEMP}/${CLOUD_FOLDER}/${DIR_SYNC}/"
    # shellcheck disable=SC2059
    printf   ", [${CLOUD_FOLDER}/${FILE_EXCLUDES}]"
    cp       "${DIR_LOCAL}/${FILE_EXCLUDES}"          "${DIR_LOCAL}/${DIR_TEMP}/${CLOUD_FOLDER}/${DIR_SYNC}/"
    printf   ". Ок\n"
...
#
# Если CMD_USER в списке комманд требующих синхронизацию, то вызываем find_sync_dir
#
...
echo   "╔═════════════╤═════════════════════════════════════════════════════════════════╗"
printf "║  MY:        │  %-58s  √  ║\n" "${MY_NAME}"
printf "║  CMD LOCAL  │  %-58s  √  ║\n" "${CMD_USER}"
printf "║  CMD CLOUD  │  %-58s  √  ║\n" "${CMD_CLOUD}"
...
if [ "${CMD_USER}" = "${SYNC_CMD_DL}" ]; then
...
elif [ "${CMD_USER}" = "${SYNC_CMD_UP}" ]; then

    if [ "${CMD_CLOUD}" = "${SYNC_CMD_REGULAR}" ]; then
...
elif [ "${CMD_USER}" = "${SYNC_CMD_DL_INIT}" ]; then
...
elif [ "${CMD_USER}" = "${SYNC_CMD_UP_INIT}" ]; then
    if [ "${CMD_CLOUD}" = "${SYNC_CMD_REGULAR}" ]; then
...
elif [ "${CMD_USER}" = "${SYNC_CMD_UP_EDIT}" ]; then
    if [ "${CMD_CLOUD}" = "${SYNC_CMD_PAUSE}" ]; then
...
elif [ "${CMD_USER}" = "${SYNC_CMD_REGULAR}" ] || [ -z "${CMD_USER}" ]; then
    if [ "${CMD_CLOUD}" = "${SYNC_CMD_DL_INIT}" ]; then
    ...
    elif [ "${CMD_CLOUD}" = "${SYNC_CMD_PAUSE}" ]; then
    ...
    elif [ "${CMD_CLOUD}" = "${SYNC_CMD_REGULAR}" ]; then
...
elif [ "${CMD_USER}" = "${SYNC_CMD_PAUSE}" ]; then
...
elif [ "${CMD_USER}" = "${SYNC_CMD_UNPAUSE}" ]; then
    ...
    exit_with_msg "Не известная команда пользователя ${CMD_USER}." 2;
