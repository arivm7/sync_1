...
#
COLOR_FILENAME="\033[1;36m"     # Для вывода имён файлов и путей (бирюзовый темнее)
COLOR_STATUS="\033[0;36m"       # Терминальный цвет для вывода переменной статуса (бирюзовый)
COLOR_USAGE="\033[1;34m"        # Терминальный цвет для вывода подсказки по использованию (синий ярче)
COLOR_INFO="\033[0;34m"         # Терминальный цвет для вывода информации (об ошибке или причине выхода) (синий)
...
print_banner() {
echo -e "$(cat <<EOF
${COLOR_WARNING}╔═══════════════════════════════════════════════════════════════════════════════╗${COLOR_OFF}
${COLOR_WARNING}║                                                                               ║${COLOR_OFF}
${COLOR_WARNING}║${COLOR_OFF}                                   ${COLOR_WARNING}ВАЖНО:${COLOR_OFF}                                      ${COLOR_WARNING}║${COLOR_OFF}
${COLOR_WARNING}║                                                                               ║${COLOR_OFF}
${COLOR_WARNING}║${COLOR_OFF}        ${COLOR_INFO}Этот ${COLOR_WARNING}хост считается "главным"${COLOR_OFF}${COLOR_INFO} в вашей системе синхронизации.${COLOR_OFF}           ${COLOR_WARNING}║${COLOR_OFF} 
${COLOR_WARNING}║${COLOR_OFF}              ${COLOR_INFO}Все изменения на этом хосте в указанных папках${COLOR_OFF}                   ${COLOR_WARNING}║${COLOR_OFF}
${COLOR_WARNING}║${COLOR_OFF}     ${COLOR_INFO}будут автоматически отправляться на сервер командой${COLOR_OFF} ${COLOR_WARNING}UP_INIT${COLOR_INFO}/${COLOR_WARNING}UP_EDIT${COLOR_OFF}       ${COLOR_WARNING}║${COLOR_OFF}
${COLOR_WARNING}║                                                                               ║${COLOR_OFF}
${COLOR_WARNING}╚═══════════════════════════════════════════════════════════════════════════════╝${COLOR_OFF}
EOF
)";
